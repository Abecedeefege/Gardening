...
function openSpeciesFromHash() {
  ...
  if (typeof findPlantByCode === 'function' && findPlantByCode(code)) {
...
async function writePushSubscriptionDoc(doc, message) {
  // 1) backend (sin PAT) · 2) PAT del device
  const viaApi = await apiSync({ op: 'push_subscription', doc, device: loadDeviceName() || 'browser' });
  if (viaApi && viaApi.ok) return;
  await ghWriteJsonFile(PUSH_SUB_PATH, doc, message);
}
async function uploadPushSubscription(sub) {
  await writePushSubscriptionDoc({
...
// del día siguiente.
// ------------------------------------------------------------
// Loguea eventos de engagement del sitio principal (clicks de notificación,
// visitas) vía la función serverless /api/feedback — SIN PAT. Antes esto
// dependía del PAT (loadGitHubToken) y en la PWA el PAT no está disponible,
// así que las aperturas de notificación a index.html NO se medían (bug que
// hizo ver "0 engagement" el 14/06). Ahora se encolan en el outbox (igual que
// engage.js) y se postean; el abrir una notificación SIEMPRE cuenta.
async function logEngagementEvents(events) {
  if (!events || !events.length) return;
  const KEY = 'jardineando_engage_outbox_v1';
  const stamped = events.map(e => Object.assign({ _id: Date.now().toString(36) + Math.random().toString(36).slice(2, 8) }, e));
  try {
    const cur = JSON.parse(localStorage.getItem(KEY) || '[]');
    localStorage.setItem(KEY, JSON.stringify(cur.concat(stamped).slice(-200)));
  } catch (e) {}
  drainEngageOutbox();
}

// Postea el outbox (de este sitio y de las landings engage/*) a /api/feedback.
// Sin PAT. Dedup por _id lo hace el endpoint. Se llama al cargar y tras cada log.
async function drainEngageOutbox() {
  const KEY = 'jardineando_engage_outbox_v1';
  let raw;
  ...
  try {
    const r = await fetch('/api/feedback', {
      ...
      body: JSON.stringify({ events: raw, device: loadDeviceName() || 'device' }),
    });
    if (r.ok) { localStorage.removeItem(KEY); console.log(`[engage] ${raw.length} eventos posteados a /api/feedback`); }
  } catch (e) { console.warn('[engage] /api/feedback falló (quedan en outbox):', e); }
}
...
// Subida vía /api/tarea (Vercel, token del SERVIDOR): el browser NO necesita
// PAT. Devuelve el JSON de respuesta, o null si el endpoint no existe en este
// hosting (GitHub Pages, file://) o no responde — en ese caso el caller cae al
// camino viejo con el PAT del device. Un 4xx/5xx del API sí es error (se lanza).
// _apiAvailable: null = no probado, true = hay backend (Vercel), false = no
// hay (GitHub Pages / file://). Con backend, el sitio escribe SIN PAT desde
// cualquier dispositivo; el PAT del device queda solo como fallback.
let _apiAvailable = null;
async function apiPost(path, payload) {
  if (!/^https?:$/.test(location.protocol)) { _apiAvailable = false; return null; }
  let r;
  try {
    r = await fetch(path, {
      method: 'POST',
  ...
  if (r.status === 404 || r.status === 405) { _apiAvailable = false; return null; }
  const ct = r.headers.get('content-type') || '';
  if (!ct.includes('application/json')) { _apiAvailable = false; return null; } // GitHub Pages devuelve HTML de 404
  _apiAvailable = true;
  const data = await r.json().catch(() => ({}));
  if (!r.ok) {
    const hint = data.error === 'token_vencido' ? ' — el token del servidor venció: avisale a Claude' : '';
    throw new Error(`${path}: HTTP ${r.status} — ${data.error || ''} ${data.detail || ''}${hint}`.trim());
  }
  return data;
}
async function apiUploadPhoto(payload) { return apiPost('/api/tarea', payload); }
async function apiSync(payload) { return apiPost('/api/sync', payload); }

// Sondeo barato al cargar: ¿existe /api/sync acá? (200 o 502 = existe; 404/HTML = no)
async function probeApi() {
  if (!/^https?:$/.test(location.protocol)) { _apiAvailable = false; return false; }
  try {
    const r = await fetch('/api/sync', { cache: 'no-store' });
    const ct = r.headers.get('content-type') || '';
    _apiAvailable = ct.includes('application/json');
  } catch (e) { /* sin red: no concluimos */ }
  return _apiAvailable;
}
probeApi();

// ¿Este dispositivo puede escribir al repo? Con backend sí, siempre; sin
// backend solo si el usuario configuró su PAT.
function canWrite() {
  return _apiAvailable !== false || !!loadGitHubToken();
}

function friendlyUploadError(err) {
  const msg = err.message || String(err);
  if (/HTTP 401/.test(msg) && /Bad credentials/i.test(msg)) {
    return `<p>El token de GitHub guardado en este dispositivo <strong>venció o fue revocado</strong> (GitHub respondió 401). Generá uno nuevo y pegalo en ⚙️ Configuración → Token de GitHub.</p><p style="font-size:0.78rem;color:#666">Detalle técnico: ${msg}</p>`;
  }
...
async function uploadPendingSpeciesPhoto() {
  ...
  try {
    ...
    // 1) Backend serverless (sin PAT). 2) Fallback: PAT del device (GitHub Pages).
    const viaApi = await apiUploadPhoto({
      type: 'species_photo', plant_code: plantId, base64,
      text: note || null, device: loadDeviceName() || 'desconocido',
    });
    let repoPath = path;
    if (viaApi && viaApi.repo_path) {
      repoPath = viaApi.repo_path;
    } else {
    ...
    const fileUrl = `https://github.com/${GH_REPO}/blob/main/${repoPath}`;
    result.innerHTML = `
    ...
    const detail = friendlyUploadError(err);
    result.innerHTML = `
...
function scheduleSyncFlush() {
  if (!canWrite()) return;  // ni backend ni token: no hay sync write
...
async function flushSync(retries = 5) {
  ...
  updateSyncStatus();
  try {
    // 0. Backend (sin PAT): el servidor hace el merge LWW y devuelve el resultado.
    const viaApi = await apiSync({ op: 'states_merge', tasks: loadStates(), device: loadDeviceName() || 'browser' });
    if (viaApi && viaApi.tasks) {
      saveStates(viaApi.tasks);
      _stateDirty = false;
      _syncInProgress = false;
      updateSyncStatus({ ok: true });
      return;
    }
    if (!loadGitHubToken()) throw new Error('Sin backend ni GitHub PAT: no se puede sincronizar');
...
async function uploadPendingPhoto() {
  ...
  try {
    ...
    // 0. Backend serverless (sin PAT): deja la foto en el thread de la tarea
    //    y en uploads.json como pending — la procesa /responder-tareas.
    const viaApi = await apiUploadPhoto({
      type: 'photo', task_id: task.id, plant_code: plantId, base64,
      text: userContext || null, task_title: task.title, device: loadDeviceName() || 'desconocido',
    });
    let repoPath = path;
    if (viaApi && viaApi.photo_path) {
      repoPath = 'docs/images/' + viaApi.photo_path;
    } else {
...
document.getElementById('btn-text-ask-ai')?.addEventListener('click', async () => {
  ...
  try {
    ...
    const viaApi = await apiSync({ op: 'uploads_append', bucket: plantId, entry, device: loadDeviceName() || 'browser' });
    if (!viaApi) {
      const uploadsPath = 'docs/uploads.json';
      const { sha, data } = await ghReadJsonFile(uploadsPath);
      const idx = data || {};
...
async function flushUserTasksToRepo(retries = 3) {
  if (!canWrite()) return;
  try {
    const viaApi = await apiSync({ op: 'user_tasks_merge', tasks: USER_TASKS, device: loadDeviceName() || 'browser' });
    if (viaApi && viaApi.tasks) {
      USER_TASKS = viaApi.tasks;
      saveUserTasksLocal();
      return;
    }
    if (!loadGitHubToken()) throw new Error('Sin backend ni GitHub PAT: no se puede sincronizar');
...
async function submitCompose() {
  ...
  try {
    ...
    if (composeState.blob) {
      ...
      const viaApi = await apiSync({ op: 'photo_put', bucket: bucketKey, filename, base64, device: loadDeviceName() || 'browser' });
      if (!viaApi) await ghPutFile(photoPath, base64, `compose: foto para ${id}`);
    }
    ...
    const viaApiIdx = await apiSync({ op: 'uploads_append', bucket: bucketKey, entry: uploadEntry, device: loadDeviceName() || 'browser' });
    if (!viaApiIdx) {
      const { sha, data } = await ghReadJsonFile(uploadsPath);
      const idx = data || {};
...
# ============================================================
# SPLASH «Hora dorada — amanecer» — driver de producción (solo Home).
# Mecánica idéntica a la del splash anterior (elegida y validada):
#  · sessionStorage.jardineando_splash_v1: UNA vez por sesión
#  · amanecer atado a la carga real: 1× mientras parsea, WARP× en window.load,
#    retención en 96% si la carga demora — el progreso nunca miente
#  · tap/click = saltar; al terminar el remate, fade + remove() del DOM
# Escena nueva (pedido del usuario 04/07): foto real del fondo en otoño que
# arranca de NOCHE (grade multiply + estrellas) y amanece — hora azul, dorada,
# color natural — con bloom, barrido de luz sobre la pileta, lens flare y
# hojas cayendo en 2 planos que entran con la primera luz.
# ============================================================
SPLASH_JS = r"""
(()=>{
'use strict';
...
const img=$('jj-phimg'),bimg=$('jj-bloomimg'),bloom=$('jj-bloom'),
      grF=$('jj-flat'),grN=$('jj-night'),grN2=$('jj-night2'),grB=$('jj-blue'),
      grG=$('jj-gold'),grR=$('jj-rose'),vig=$('jj-vig'),starBox=$('jj-stars'),
      sweep=$('jj-sweep'),flare=$('jj-flare'),lvF=$('jj-leaves'),lvB=$('jj-leaves2'),
      pctW=$('pctW'),pct=$('pct'),copy=$('copy');
const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
const DUR=5000,WARP=2.6,GATE=.96;
const clamp=(x,a,b)=>x<a?a:x>b?b:x;
const ss=(a,b,x)=>{const k=clamp((x-a)/(b-a),0,1);return k*k*(3-2*k)};
const KEYS=[[0,0],[.13,.05],[.3,.4],[.5,.64],[.62,.74],[.72,.78],[.84,.81],[.92,.92],[1,1]];
function curve(u){
  if(u>=1)return 1;
...
const COPY=['Todavía es de noche en el fondo…',
'Clarea detrás del liquidámbar…',
'Primera luz — arrancan a caer las hojas…',
'Amanece dorado sobre la pileta.'];
const CT=[0,.25,.5,.75].map(x=>x*DUR);
const LEAF='<svg width="__S__" height="__S__" viewBox="0 0 20 20"><path d="M10 1 L13 7 L19 8 L14 12 L16 19 L10 15 L4 19 L6 12 L1 8 L7 7 Z" fill="__C__" opacity=".9"/></svg>';
const LCOL=['#c1502e','#d98a3c','#a63c22','#e0a84e','#8a4a2a'];

let stars=[],raf=0,ci=0,fc=0,copyTimer=0,leafy=false,
...
function buildStars(){
  ...
  for(let i=0;i<32;i++){
    const o=document.createElement('i');o.className='jj-st';
    const sz=Math.random()>.82?2.6:1.6;
    o.style.width=o.style.height=sz+'px';
    o.style.left=(Math.random()*100).toFixed(1)+'%';
    o.style.top=(Math.random()*92).toFixed(1)+'%';
    o.style.animationDuration=(2+Math.random()*3).toFixed(2)+'s';
    o.style.animationDelay=(-Math.random()*4).toFixed(2)+'s';
    frag.appendChild(o);
    stars.push({el:o,th:.14+Math.random()*.4});
  }
  starBox.appendChild(frag);
}
function buildLeaves(){
  lvF.textContent='';lvB.textContent='';
  for(let i=0;i<16;i++){
    const el=document.createElement('span');el.className='jj-lf';
    const back=i%2===0;
    const size=back?9+Math.random()*5:13+Math.random()*8;
    el.innerHTML=LEAF.replace(/__S__/g,size.toFixed(0)).replace('__C__',LCOL[i%LCOL.length]);
    el.style.left=(Math.random()*100).toFixed(1)+'%';
    el.style.animationDuration=(back?9:6.5+Math.random()*3).toFixed(2)+'s';
    el.style.animationDelay=(-Math.random()*9).toFixed(2)+'s';
    const svg=el.querySelector('svg');
    if(svg)svg.style.animationDuration=(1.6+Math.random()*1.6).toFixed(2)+'s';
    (back?lvB:lvF).appendChild(el);
  }
}
...
function render(p,full){
  if(!reduced){
    const sc=1.15-.11*p;
    const tx=(1-p)*2.4-1.2;
    const t=`scale(${sc.toFixed(4)}) translateX(${tx.toFixed(2)}%)`;
    img.style.transform=t;bimg.style.transform=t;
  }
  grN.style.opacity=(1-ss(.12,.6,p)).toFixed(3);
  grN2.style.opacity=(.45*(1-ss(.12,.55,p))).toFixed(3);
  grF.style.opacity=(.75*(1-ss(.15,.6,p))).toFixed(3);
  grB.style.opacity=(ss(.28,.48,p)*(1-ss(.58,.75,p))).toFixed(3);
  grG.style.opacity=(ss(.5,.78,p)*(1-.4*ss(.94,1,p))).toFixed(3);
  grR.style.opacity=ss(.58,.9,p).toFixed(3);
  bloom.style.opacity=(.24*ss(.5,.8,p)*(1-.5*ss(.94,1,p))).toFixed(3);
  flare.style.opacity=(ss(.6,.82,p)*(1-ss(.95,1,p))).toFixed(3);
  vig.style.opacity=(1-.5*p).toFixed(3);
  const sw=ss(.66,.9,p);
  sweep.style.opacity=(sw>0&&sw<1?.35+.45*Math.sin(sw*Math.PI):0).toFixed(3);
  sweep.style.transform=`skewX(-14deg) translateX(${(sw*320).toFixed(1)}%)`;
  if(full){
    for(const st of stars)st.el.style.opacity=(1-ss(st.th-.05,st.th+.04,p)).toFixed(3);
  }
  if(!reduced&&!leafy&&p>=.5){leafy=true;splash.classList.add('leafy')}
  const n=Math.round(p*100);
  pct.textContent=n;
  pctW.setAttribute('aria-valuenow',n);
  pctW.style.opacity=p>.04?'1':'0';
}

function frame(now){
  if(!last)last=now;
  /* dt sin tope: si el main thread se traba parseando, el arco salta el tiempo real */
...
function finale(){
  finaleOn=true;
  setTimeout(()=>splash.classList.add('washing'),200);
  setTimeout(()=>{splash.classList.add('reveal');splash.classList.remove('leafy')},720);
  setTimeout(dismiss,2450);
}
function dismiss(){
