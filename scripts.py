...
async function apiUploadPhoto(payload) {
  ...
  const ct = r.headers.get('content-type') || '';
  if (!ct.includes('application/json')) return null; // GitHub Pages devuelve HTML de 404
  const data = await r.json().catch(() => ({}));
  if (!r.ok) throw new Error(`/api/tarea: HTTP ${r.status} — ${data.error || ''} ${data.detail || ''}`.trim());
  return data;
}
