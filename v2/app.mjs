...
function welcomeScreen(){
 root.innerHTML=`<main class="welcome-screen"><header>${brand()}</header><section class="welcome-body"><div class="welcome-art" aria-hidden="true">${icon('garden')}<span class="welcome-spark">✧</span></div><h1>Bienvenido a<br>Jardineando.</h1><p>Un lugar para tus plantas y sus cuidados.<br>Organizá tus ambientes, conocé cada especie<br class="welcome-break"> y acompañá su crecimiento.</p></section><footer>${btn('Empezar mi jardín →','welcome-start','primary')}<p>Tu primer ambiente. Tu primera planta.</p></footer></main>`;
}
function onboarding(){
...
function render(){if(!account||!state){login();return;}if(!activeGarden()){shell(gate());return;}if(own()&&!state.welcomeSeen&&(demo||!plants().length)){welcomeScreen();return;}if(needsOnboarding()){onboarding();return;}const screens={today:home,garden,detail,add,ask,settings,'new-task':newTask};shell((screens[view]||home)());}
...
async function action(name,node){
 if(name==='welcome-start'){state.welcomeSeen=true;await persist();render();window.scrollTo(0,0);return;}
